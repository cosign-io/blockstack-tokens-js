...
export function checkConversion (value, type) {
  ...
  if (type.indexOf('uint') > -1 && uuidRegex.test(value)) {
    return new BN(value.replace(new RegExp('-'.replace(/([.*+?^=!:${}()|\[\]\/\\])/g, "\\$1"), 'g'), ''), 16);
  }
  //replace date
  ...
  if (type.indexOf('bytes') > -1 && type.indexOf('[]') == -1 && !hexRegex.test(value)) {
    var arr = [];
